<?php	if(!isset($_GET["buddy"])){header("Location:buddyChat"); exit(); } else{$chatWith=$_GET["buddy"]; } ?>
<html>
<head>
	<meta charset="utf-8">
...
<body class="cbp-spmenu-push">
		...
		<div id="navContainer" class="visible-md visible-lg "><?php include("navContainer.php"); $_SESSION["chatWithId"]=$chatWith; ?></div>
					  ...
					  <div class="box box-primary direct-chat direct-chat-primary">
						<div class="box-header with-border">
						  <h3 class="box-title">Buddy Chat</h3>
							<div class="box-tools pull-right">
								<a href="buddyChat" type="button" class="btn btn-box-tool" data-widget="remove"><i class="glyphicon glyphicon-remove"></i> Close</a>
							</div>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<?php 
								...
								if(!mysqli_num_rows($get)>0){echo '<div class="alert alert-info">There Is No Message For Now</div>';}
								else{ while($data=mysqli_fetch_array($get)){
								...
								$text.='<span class="direct-chat-timestamp pull-left">'.date('m/d/Y g:iA', strtotime($data["datePosted"])).'</span>';
								$text.='</div><div class="direct-chat-text">'.$data["message"].'</div></div>'; }
								
								else{
								$text='<div class="direct-chat-msg"><div class="direct-chat-info clearfix">';
								$text.='<span class="direct-chat-name pull-left">'.$data["userName"].'</span>';
								$text.='<span class="direct-chat-timestamp pull-right">'.date('m/d/Y g:iA', strtotime($data["datePosted"])).'</span>';
								$text.='</div><div class="direct-chat-text">'.$data["message"].'</div></div>'; }
								echo $text;
								} }
	...
	<div id="pageInfo" class="hidden">buddyChatPage</div>
		...
		<script>getMessageInterval=setTimeout(getBuddyMessage,2000); changeMsgStatus();</script>
		<script type="text/javascript">
  		var meIdd=getCookie("chatWithId");
  		if(meIdd=="" || meIdd==null){
  			document.cookie="chatWithId=; expires=Thu, 18 Dec 1999 12:00:00 UTC; path=/";	
			setCookie("chatWithId",<?php echo $chatWith;  ?>,"2");
  		}
  	</script>
</body>
</html>